...
class Dao:
    ...
    def __new__(cls, mongodb_settings=None, co2class=None):
        """
        Create connection to db in a singleton manner
        Params (only needed for 1st constructor call):
            - MONGODD_SETTINGS: dict that contains host, port and db_name
            - col2class: dict that maps business objects to collection/table names
        """
        if cls._instance is None:
            if (mongodb_settings is None) or (co2class is None):
                raise Exception(
                    "DAO constructor needs parameters to be initialized (cf. readme file)"
                )
            else:
                # DB-business objects mapping
                cls.col2class = co2class
                cls.class2col = {
                    cls.col2class[col]: col for col in cls.col2class.keys()
                }
                ...
                try:
                    ...
                    raise Exception(f"Could not connect to MongoDB at {host}:{port}")
    ...
    def create_objects(cls, class_to_init, data, key_id="id", is_fk_obj=False):
        """
        Add registers (lines) to associated collection.
        Note: if the collection was not instantiated yet it creates one
        """
        col_name = cls.class2col[class_to_init]
    ...
    def read_objects(cls, class_to_init, id_list, key_id="id", is_fk_obj=False):
        """
        return dict of id-object pairs
        """
        objects = {}
        col_name = cls.class2col[class_to_init]
        col_list = cls._db.list_collection_names()
    ...
    def update_objects(cls, class_to_init, data, key_id="id", is_fk_obj=False):
        """
        updating is about a 100 times slower than inserting, so make sure that's necessary (i.e. the object already exists in collection)
        inputs:
    ...
    def drop_db(cls):
        cls._client.drop_database(cls._db.name)
